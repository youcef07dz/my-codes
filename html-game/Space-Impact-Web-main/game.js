...
function playSound(type) {
    ...
    switch(type) {
        ...
        case "gameOver":
            ...
            osc.frequency.exponentialRampToValueAtTime(98, audioCtx.currentTime + 0.8);
            gain.gain.setValueAtTime(0.2 * volume, audioCtx.currentTime);
...
function animate(timestamp) {
    ...
    if (isMainScreen) mainScreen();
        ...
        if (autoFireEnabled && timestamp - lastAutoFireTime > autoFireCooldown) {
            if (!keys.space.pressed) {
                currentLevel.playerProjectiles.push(new Projectile(true, currentLevel.player));
                keys.space.pressed = true;
                playSound("shoot");
                setTimeout(() => { keys.space.pressed = false; }, 50);
            }
...
if (autoFireToggle) {
    let autoFireFlag = false;
    autoFireToggle.addEventListener("click", () => {
        autoFireFlag = !autoFireFlag;
        autoFireToggle.classList.toggle("toggle-on", autoFireFlag);
        autoFireToggle.querySelector(".toggle-ball").classList.toggle("toggle-ball-on", autoFireFlag);
        autoFireEnabled = autoFireFlag;
    });
}